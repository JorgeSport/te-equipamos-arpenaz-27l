...
article_fn = r'''function renderArticle(id){const n=NEWS.find(x=>x.id===Number(id));if(!n){history.replaceState({},'','./');renderPortal();return}$('#portal').classList.add('hidden');$('#articleView').classList.remove('hidden');document.title=`${n.title} — Te Equipamos News`;const p=[n.summary,n.details||'Te Equipamos ha preparado este resumen para ofrecer una lectura rápida de la actualidad.','Este contenido ha sido redactado de forma independiente a partir de la información publicada por la fuente indicada. Para consultar todos los detalles, contexto y posibles actualizaciones, abre la noticia original.'];$('#article').innerHTML=`<img class="articleHero" src="${n.image}" alt="${esc(n.title)}"><div class="articleBody"><span class="kicker">${n.category} · Actualidad</span><h1>${esc(n.title)}</h1><p class="deck">${esc(n.summary)}</p><div class="byline"><strong>Te Equipamos News</strong><span>•</span><span>Fuente: ${esc(n.source)}</span><span>•</span><span>${n.time}</span></div><div class="copy">${p.map((x,i)=>`${i===1?'<h2>Lo más importante</h2>':''}<p>${esc(x)}</p>`).join('')}<a class="sourceBtn" href="${n.url}" target="_blank" rel="noopener noreferrer">Leer noticia original en ${esc(n.source)} ↗</a></div><div class="note"><strong>Transparencia editorial:</strong> Te Equipamos resume y contextualiza la información. La autoría y el contenido completo pertenecen al medio enlazado. Imagen de portada ilustrativa.</div></div>`;$('#related').innerHTML=NEWS.filter(x=>x.id!==n.id).slice(0,4).map(x=>`<div class="rel" data-article="${x.id}"><b>${esc(x.title)}</b><small>${x.category} · ${x.time}</small></div>`).join('')}
'''
